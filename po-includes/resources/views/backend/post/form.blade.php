<div class="row">
	...
	<div class="col-md-3">
		...
		<div class="form-group">
			{!! Form::label('tag', __('Tag').' *', ['class' => 'control-label']) !!}
			<select class="select-style form-control" id="type" name="tag">
					@if($formMode == 'edit')
						<option value="{{ $post->tag }}">{{ __('general.selected') }} {{ ucfirst($post->tag) }}</option>
					@endif
		...
		<div class="form-row">
			...
			<div class="form-group col-md-6">
				{!! Form::label('active', __('post.status').' *', ['class' => 'control-label']) !!}
				<select class="select-style form-control" id="active" name="active">
					@if($formMode == 'edit')
						<option value="{{ $post->active }}">{{ __('general.selected') }} {{ $post->active == 'Y' ? __('post.active') : __('post.unpublish') }}</option>
					@endif
					@if (Auth::user()->hasRole('superadmin'))
                        <option value="Y">{{ __('post.publish') }}</option>
					    <option value="N">{{ __('post.unpublish') }}</option>
					@else
					    <option value="N">{{ __('post.unpublish') }}</option>
					@endif
				</select>
