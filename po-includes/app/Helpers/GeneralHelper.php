...
if (!function_exists('getCommentsActive')) {
    function getCommentsActive($post_id)
    {
		$result = Comment::select('post_id', 'active')->Where('post_id','=', $post_id)->andWhere('active','=','Y')->get();
		if ($result) {
			return $result->value;
		} else {
			return '';
...
if (!function_exists('getOPDPages')) {
    function getOPDPages()
    {
		$result = Pages::select('id', 'title', 'seotitle', 'picture')->where('title', 'like', '%' . 'Dinas' . '%')->orWhere
		    ('title', 'like', '%' . 'Badan' . '%')->orWhere
		    ('title', 'like', '%' . 'Satuan' . '%')->orWhere
		    ('title', 'like', '%' . 'Inspektorat' . '%')->orWhere
		    ('title', 'like', '%' . 'Dewan' . '%')->get();
		return $result;
	}
}

if (!function_exists('getTotalVisitor')) {
...
if (!function_exists('getUniqueVisitor')) {
    function getUniqueVisitor()
    {

	$result = DB::table('visitors')
        ->select(DB::raw('COUNT(*) AS uni'))
        ->get();
		return $result;
	}
}
...
if (!function_exists('getUnpublishedComments')) {
    function getUnpublishedComments()
    {
		$result = Comment::select('id', 'parent', 'post_id', 'name', 'email', 'content', 'active', 'status', 'created_by', 'updated_by', 'created_at', 'updated_at')->where('active', '=', 'Y')->get();
		return $result;
	}
}
